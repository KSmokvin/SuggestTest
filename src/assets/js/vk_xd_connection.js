...
VK.initXDConn = function() {
  ...
  VK._Rpc = new fastXDM.Client({
    ...
    runCallback: function(args) {
      ...
      if (VK.isFunc(VK._callbacks[eventName]))
        VK._callbacks[eventName].apply(VK, args);
    },
      ...
      var calcHeight = function() {
        ...
        if (height) {
          if (window.getComputedStyle !== undefined) {
            ...
            height += parseInt(
              st.getPropertyValue("margin-top").replace("px", "")
            );
            height += parseInt(
              st.getPropertyValue("margin-bottom").replace("px", "")
            );
          } else {
            height += parseInt(
              document.body.currentStyle["marginTop"].replace("px", "")
            );
            height += parseInt(
          ...
          if (!VK._openApiAttached) {
            VK.callMethod("_getAppInfo", function(data) {
              var baseDomain =
                VK._domain &&
                VK._domain.base &&
                VK._domain.base.match(/^(\w+\.)*vk.com$/)
                  ? VK._domain.base
                  : "https://vk.com";
              VK._apiId = data[0];
              VK._browserHash = data[1];
              VK.addScript(baseDomain + "/js/api/openapi.js?166");
            });
...
/* Obsolete methods */
VK.External = {
  showPaymentBox: function(a) {
    VK.callMethod("showPaymentBox", a);
  },
  showSettingsBox: function(a) {
    VK.callMethod("showSettingsBox", a);
  },
  ...
  resizeWindow: function(b, a) {
    VK.callMethod("resizeWindow", b, a);
  },
  scrollWindow: function(b, a) {
    VK.callMethod("scrollWindow", b, a);
  },
  setLocation: function(a, b) {
    VK.callMethod("setLocation", a, b);
  },
  setTitle: function(a) {
  ...
  showMerchantPaymentBox: function(a) {
    VK.callMethod("showMerchantPaymentBox", a);
  }
};
